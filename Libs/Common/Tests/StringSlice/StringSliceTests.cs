...
namespace RhoMicro.LogoSyn.Libs.Common.Tests
{
	[TestClass]
	public class StringSliceTests
	{
		...
		public void InvalidSlice_Value(String value)
		{
			if (value == null)
			{
				_ = Assert.ThrowsException<ArgumentNullException>(() => value!.Slice());
			}
			else
			{
				_ = Assert.ThrowsException<ArgumentOutOfRangeException>(() => value.Slice());
			}
		}
		...
		public void InvalidSlice_Value_Length(String value, Int32 length)
		{
			if (value == null)
			{
				_ = Assert.ThrowsException<ArgumentNullException>(() => value!.Slice(length));
			}
			else
			{
				_ = Assert.ThrowsException<ArgumentOutOfRangeException>(() => value.Slice(length));
			}
		}
		...
		public void InvalidSlice_Value_Start_Length(String value, Int32 start, Int32 length)
		{
			if (value == null)
			{
				_ = Assert.ThrowsException<ArgumentNullException>(() => value!.Slice(start, length));
			}
			else
			{
				_ = Assert.ThrowsException<ArgumentOutOfRangeException>(() => value.Slice(start, length));
			}
		}
		#endregion
		...
		private static void AssertSlicesInequal(IStringSlice x, IStringSlice y)
		{
			Assert.IsTrue(x.Equals(x));
