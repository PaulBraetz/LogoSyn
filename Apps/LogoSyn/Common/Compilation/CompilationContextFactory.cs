...
internal sealed class CompilationContextFactory : ApplicationContextFactoryBase
{
	...
	static CompilationContextFactory()
	{
		...
		_ = _parameters.TryAdd("c", "compile", "When set, source documents may be compiled.", s => s == null);

		_ = _parameters.TryAdd("cs", "compileSource", "Supplies the .ls source file path", File.Exists);
		_ = _parameters.TryAdd("cm", "compileManifests",
		...
		_ = _parameters.TryAdd("ct", "compileTarget", "Supplies the target file to which to write the compilation result.", s => !String.IsNullOrEmpty(s));
		_ = _parameters.TryAdd("ce", "compileError", "Supplies the file to which to write error details, should any arise.", s => !String.IsNullOrEmpty(s));
	}
