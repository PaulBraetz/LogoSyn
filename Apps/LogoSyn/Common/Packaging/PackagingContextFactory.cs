...
namespace RhoMicro.LogoSyn.Apps.LogoSyn.Common.Packaging
{
	internal sealed class PackagingContextFactory : ApplicationContextFactoryBase
	{
		static PackagingContextFactory()
		{
			...
			_parameters.TryAdd("p", "package", "When set, packages may be managed.", s => s == null);
			_parameters.TryAdd("ph", "packageHeadless", "When set, packages may be managed headlessly.", s => s == null);

			_parameters.TryAdd("pi", "packageInfo", "Supplies the package information file for creating a package.", File.Exists);
			_parameters.TryAdd("pd", "packageDirectory", "Supplies the package data directory containing all files required for creating a package.", Directory.Exists);
			_parameters.TryAdd("pt", "packageTarget", "Supplies the target directory for the package created.");
			_parameters.TryAdd("pm", "packageManifest", "Supplies the manifest file for registering the package created.", s => !String.IsNullOrEmpty(s));
		}
		...
		private static void CreatePackage(IArgumentCollection arguments, IPackagingContext context)
		{
			...
			if (hasInfo && hasDirectory)
			{
				...
				if (arguments.TryGet("pm", s => new FileInfo(s!), out var manifestFile))
				{
					...
					manifest.TryAddPackageFile(packageFile);
