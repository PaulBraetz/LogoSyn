...
namespace RhoMicro.LogoSyn.Apps.Parser.CSharpParser
{
	internal class Program
	{
		static Program()
		{
			...
			_parameters.TryAdd("o", "offset", "Supplies the number of offset lines to add before the document.", s => Int32.TryParse(s, out var _));
			_parameters.TryAdd("s", "source", "Supplies the source document path.", File.Exists);
			_parameters.TryAdd("t", "target", "Supplies the target file to which to write the parsed document.", s => !String.IsNullOrWhiteSpace(s));
		}
